...
export default class MerchantController extends Controller {
  ...
  public async index() {
    const { ctx } = this;
    const list = await ctx.service.merchant.index.list(ctx.query);
    ctx.success(list);
  }
  /** 不分页列表 */
  ...
  public async list() {
    const { ctx } = this;
    const list = await ctx.service.merchant.index.select();
    ctx.success(list);
  }
  ...
  public async save() {
    ...
    const ret = await ctx.service.merchant.index.save(params);
    if (ret.code === 0) {
      ctx.success();
    } else {
  ...
  public async update() {
    const { ctx } = this;
    const params = ctx.request.body;
    const ret = await ctx.service.merchant.index.update(params);
    if (ret.code === 0) {
      ctx.success();
    } else {
      ctx.fail(ret.message, ret.code);
  ...
  public async remove() {
    const { ctx } = this;
    const ret = await ctx.service.merchant.index.remove(ctx.params.id);
    if (ret.code === 0) {
      ctx.success();
    } else {
      ctx.fail(ret.message, ret.code);
